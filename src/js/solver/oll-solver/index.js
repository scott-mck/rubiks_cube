...
import { getCubieColors, getRelativeDirection } from '../../utils/relative-finder'
...
class OllSolver {
	constructor() {
		...
		this._cornerPositionsForR = {
			...
			'u f u l': 'j j i j k j i f k f k g i h', // #28
			'u r u b': 'j k f i f k j i j n i f k j y', // #29
			'l u r b': 'u u r j j d j e j c', // #33
			'l r u b': 'j j u j k j i j j m', // #34
			...
			'f u u b': 'i j n f y f k j i y j n k', // #50
			'u u r l': 'f k f h j i f k g i', // #51
			'l f b l': 'j j k h i i g ; j j e j j d g', // #53
			'u f b u': 'j n f k j e f i i f k j j i y', // #54
			'l r u u': 'y j d f i j e e j d j j r', // #55
	...
	solve() {
		let yellows = grabber.getAllEdges().filter((edge) => {
			let data = f2lSolver.getData(edge)
			return data.edge.color.yellow === 'u'
		})
		...
		if (yellows.length === 2) {
			let faces = yellows.map((edge) => {
				...
				return data.edge.color[otherColor]
			})
	...
	async r(yellows) {
		...
		let algorithm = this._cornerPositionsForR[cornerFormation]
	...
	setupRPosition(yellows) {
		...
		let targetFace = 'f'
		let currentFace
		if (getRelativeFace(data0.edge.secondary.face, 'r') === data1.edge.secondary.face) {
			currentFace = data0.edge.secondary.face
		} else {
			currentFace = data1.edge.secondary.face
		}
	...
	getCornerFormation() {
		...
		for (let position of cornerPositions) {
			...
			let data = f2lSolver.getCornerData(corner)
			yellowFaces.push(data.corner.color.yellow)
		}
